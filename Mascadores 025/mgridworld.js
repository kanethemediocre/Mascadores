	...
	draw(viewx,viewy,xstep,ystep){
		...
		while(i<this.m){//each column
			...
			while(j<this.n){ //each row
				...
				context.fillText(this.grid[i][j].q,bordersize+xstep*i+xstep/4-viewx,bordersize+ystep*j+ystep/2-viewy);
				if (this.colors.length>1){
					context.strokeStyle = this.colors[1];
					context.beginPath();
					context.rect(bordersize+xstep*i+4-viewx,bordersize+ystep*j+4-viewy,xstep-8,ystep-8);
					context.stroke();
					}
				j++;
				}
		...
		while(i<this.players.length){
			...
			var px = bordersize + xstep*(this.players[i].x+0.125)-viewx;
			var py = bordersize + ystep*(this.players[i].y+0.125)-viewy;
